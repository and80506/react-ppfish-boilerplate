...
class SuggestPanel extends Component {
  ...
  handleKeyUp(e) {
    ...
    const len = this.state.stockList.length;
    // 往下翻
    if (keycode == 40) {
      this.setState({
  ...
  handleSearch(e, index) {
    ...
    let selectObj = this.state.stockList[selectedIndex];
  ...
  getPanel() {
    ...
    const getItems = () => {
      ...
      const panelItem = this.state.stockList.map((item, i) => {
        const selectedCls = classNames({
          'clearfix': true,
          'stock-suggest-panel-item': true,
      ...
      return (
        <ul className="stock-suggest-panel-items">
          {panelItem}
        </ul>
      );
    };
    ...
    return (
      <div className="stock-suggest-panel" style={style} onKeyUp={this.handleKeyUp}>
        {panelItems}
      </div>
    );
  }
